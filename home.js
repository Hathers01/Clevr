document.addEventListener('DOMContentLoaded', function() {
   ...
   // Muat data 'tutors' dari localStorage, atau gunakan default
    ...
    // === TAMBAHAN PENTING: LOGIKA REDIRECT DARI PROFILE ===
    // Cek apakah URL memiliki parameter ?view=myclasses
    const urlParams = new URLSearchParams(window.location.search);
    const viewParam = urlParams.get('view');

    // Jika ada parameter view='myclasses', otomatis buka halaman My Classes
    if (viewParam === 'myclasses') {
        // Sembunyikan home, tampilkan my classes
        setTimeout(() => {
            if (homePage) homePage.classList.add('d-none');
            if (tutorHomePage) tutorHomePage.classList.add('d-none');
            if (myClassesPage) {
                myClassesPage.classList.remove('d-none');
                renderMyClassesPage(); // Pastikan fungsi ini dipanggil untuk memuat data
            }
        }, 50); // Delay sedikit biar aman
    }
    // === SELESAI TAMBAHAN ===
    ...
    packagesContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('buy-clover-btn')) {
            ...
            currentCloverPackage = {
                clovers: parseInt(clovers),
                price: price
            };
        ...
        if (e.target.id === 'confirm-transfer-btn') {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    saveData(); // <-- PERBAIKAN 1: Tadinya dikomentari
                    updateCloverDisplay();
        ...
        if (e.target.classList.contains('book-from-tutor-btn')) {
            ...
            if (user.clovers >= price) {
                ...
                if (courseToBook && currentTutorIdForBooking) {
                    ...
                    saveData(); // <-- PERBAIKAN 2: Tadinya dikomentari
                    hasNewBooking = true;
                    updateNotificationDot('class');
                }
    ...
    classModal.addEventListener('click', (e) => {
        ...
        if (e.target.id === 'book-online-class-btn') {
            ...
            if (user.clovers >= price) {
                ...
                saveData(); // <-- Ini sudah benar di file asli Anda
                hasNewBooking = true;
                updateNotificationDot('class');
    ...
    buyCloverModal.addEventListener('click', (e) => {
        if (e.target === buyCloverModal) closeCloverModalBtn.click();
    });
